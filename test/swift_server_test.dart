...
void main() async {
  ...
  test(
    'routing',
    overridePrint(() async {
      var response = await getServerResponse(raw_server.$om.server, MockRequest.get('/schema'));
  ...
  test(
    '404',
    overridePrint(() async {
      var response = await getServerResponse(raw_server.$om.server, MockRequest.get('/somethingnotexisting'));
  ...
  test(
    'test action',
    overridePrint(() async {
      var response = await getServerResponse(raw_server.$om.server, MockRequest.get('/test'));
      expect(response.statusCode, 200);
  ...
  test(
    'status',
    overridePrint(() async {
      var response = await getServerResponse(raw_server.$om.server, MockRequest.get('/status'));
      expect(response.statusCode, 200);
      Map<String, dynamic> ret = response.toJson();
  ...
  test(
    'favicon',
    overridePrint(() async {
      var response = await getServerResponse(raw_server.$om.server, MockRequest.get('/favicon.ico'));
      expect(response.statusCode, 200);
